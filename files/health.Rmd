...
- Calculate bike dock ratio (GOAL: 2.0-2.5 docking spaces per bike)
- Calculate system efficiency (GOAL: 4-8 daily uses per bike)
...
```{r, eval=FALSE}
bike.use.per.day <- rides %>%
  group_by(bikeid, day.year) %>%
  dplyr::summarize(Total = n())

mean_bike.use.per.day <- bike.use.per.day %>%
...
- Define number of departures at each station, by month, week, day, and hour
- Define number of arrivals at each station, by month, week, day, and hour
...
```{r, eval=FALSE}
library(dplyr)
worst.stations <- semi_join(bottom_departures.by_month, bottom_arrivals.by_month)
worst.stations <- subset(stations, stations$id == worst.stations)
# print(unique(worst.stations$from_station_id))
```
...
- Calculate service area, per km and per mi
- Calculate number of residents in service area
- Calculate station density (GOAL: 10-16 stations per km^2 (26-42 stations per mi^2))
- Calculate bike/resident ratio (GOAL: 10-30 bikes per resident in coverage area)
```{r, eval=FALSE}
service.area_km <- length(unique(stations$id)) * .5
